...
function App() {
  ...
  useEffect(() => {
    ...
    console.log('VITE_API_URL:', import.meta.env.VITE_API_URL || 'https://alatree-ventures-assignments-1b3h.vercel.app');
  ...
  const handleCreateTestEntry = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://alatree-ventures-assignments-1b3h.vercel.app'}/api/create-test-entry/${userId}`);
  ...
  const handleCreateMultipleTestEntries = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://alatree-ventures-assignments-1b3h.vercel.app'}/api/create-test-entries/${userId}`);
      const data = await response.json();
      if (response.ok) {
